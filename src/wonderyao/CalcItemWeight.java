...
public class CalcItemWeight extends EvalFunc<DataBag> {
	...
	public DataBag exec(Tuple input) throws IOException {
		...
		Double weight = (Double)item1.get(2)*(Double)item2.get(2)/total/total;
	...
	public Schema outputSchema(Schema input){
		try{
			if(input.getFields().size() != 2 ||
				...
				throw new RuntimeException("expect input ({bag}, total: double)");
			}
